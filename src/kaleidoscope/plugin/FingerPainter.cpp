...
EventHandlerResult FingerPainter::onKeyswitchEvent(Key &mapped_key, KeyAddr key_addr, uint8_t key_state) {
  ...
  uint8_t color_index = ::LEDPaletteTheme.lookupColorIndexAtPosition(color_base_, Device.getLedIndex(key_addr));
  ...
  ::LEDPaletteTheme.updateColorIndexAtPosition(color_base_, Device.getLedIndex(key_addr), color_index);
...
EventHandlerResult FingerPainter::onFocusEvent(const char *command) {
  ...
  if (sub_command == CLEAR) {
    for (uint16_t i = 0; i < Device.numKeys() / 2; i++) {
      Device.storage().update(color_base_ + i, 0);
    }
